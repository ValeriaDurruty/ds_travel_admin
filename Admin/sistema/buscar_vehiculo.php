...
<head>
  ...
  <title>Vehiculos / Editar - Eliminar - DS Travel</title>
...
<body>
  ...
  <?php 
			...
			if(empty($busqueda))
			{ ?>
				<script> window.location.href="mod_baja-vehiculos.php"; </script>
  <?php
			}
		 ?>
    ...
    <section class="section">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Editar / Eliminar Vehículos</h5>

              <div class="search-bar" style="float: right">
                <form class="search-form d-flex align-items-center" method="GET" action="buscar_vehiculo.php">
                  ...
                  <table class="table table-hover">
                    ...
                    <?php
                        ...
                        $query = mysqli_query($conexion,"SELECT v.OID, v.nombre, v.marca, v.modelo, v.imagen, v.precioPorDia, v.capacidad, v.tipoCaja, v.aireAcondicionado, v.kilometrajeHabilitado, v.tipoCobertura, v.valoracion, v.fk_sucursal, s.localidad, (s.nombre) as nomSucursal, (p.nombre) as nomProvincia, (e.nombre) as nomEmpresa FROM vehiculo v
                        ...
                        WHERE 
                        ( v.OID LIKE '%$busqueda%' OR 
                          v.nombre LIKE '%$busqueda%' OR 
                          v.marca LIKE '%$busqueda%' OR 
                          v.modelo LIKE '%$busqueda%' OR 
                          v.precioPorDia LIKE '%$busqueda%' OR 
                          v.capacidad LIKE '%$busqueda%' OR 
                          v.tipoCaja LIKE '%$busqueda%' OR 
                          v.aireAcondicionado LIKE '%$busqueda%' OR 
                          v.kilometrajeHabilitado LIKE '%$busqueda%' OR 
                          v.tipoCobertura LIKE '%$busqueda%' OR 
                          v.valoracion LIKE '%$busqueda%' OR 
                          s.localidad LIKE '%$busqueda%' OR 
                        ...
                        while ($vehiculos = mysqli_fetch_array($query)) {   
                    ?>
                    ...
                    <tbody>
                      <tr>
                        ...
                        <td><?php echo $vehiculos['nomEmpresa']; ?></td>
                        <td><?php echo $vehiculos['localidad']; ?></td>
                        <td><img src="data:image/jpg;base64,<?php echo base64_encode($vehiculos['imagen']); ?>" width="35"/></td>
                        <td>
						            <a href="editar-vehiculos.php?id=<?php echo $vehiculos["OID"]; ?>"><button type="button" class="btn btn-primary">Editar</button></a>
						            <a href="eliminar-vehiculos.php?id=<?php echo $vehiculos["OID"]; ?>"><button type="button" class="btn btn-danger">Eliminar</button></a>	
					              </td>
                        </tr>
                        <?php }};?>
                    </tbody>
